...
public class UI {
    ...
    private static void printCapturedPieces(List<ChessPiece> capturadas){
        List<ChessPiece> white = capturadas.stream().filter(x -> x.getColor() == Colors.White).collect(Collectors.toList());
        List<ChessPiece> black = capturadas.stream().filter(x -> x.getColor() == Colors.Black).collect(Collectors.toList());
